 ...
 * @author sodok
 */
public class adminAddNewHomestayController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet adminAddNewHomestayController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet adminAddNewHomestayController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    try {
        ...
        dao.insertNewRoom(imageRoom, roomName, place, price, quantityBed, star, descriptions, detailImageRoom1, detailImageRoom2, detailImageRoom3);
        
        ArrayList<Room> rooms = dao.getRooms();
        request.setAttribute("rooms", rooms);
        request.getRequestDispatcher("adminListHomestay.jsp").forward(request, response);
    } catch (NumberFormatException e) {
        out.println("Invalid input format. Please enter again !!!");
    }
}
